...
Usage:
  ...
  /home/luwa/.conda/envs/dsclr/bin/python -m eval.generate_dual_rank_scatter_v8
"""
...
OUTPUT_NAME = "dual_rank_scatter_followir_repllama_v8"
...
def main():
    ...
    fig, ax = plt.subplots(1, 1, figsize=(6.5, 6.2), dpi=300)
    max_rank = 1000

    ax.plot([1, max_rank], [1, max_rank], color='gray', linewidth=1.5,
    ...
    for cat_docs, color, marker, label_text in [
        ...
        ax.scatter(trace_ranks, base_ranks, c=color, alpha=0.6, s=55,
    ...
    ax.scatter(synth_sat_trace, synth_sat_base, c=COLOR_SATISFYING, alpha=0.6, s=55,
               marker='D', edgecolors='white', linewidths=0.5, zorder=3)
    ...
    ax.scatter(synth_aff_trace, synth_aff_base, c=COLOR_AFFECTED, alpha=0.6, s=55,
    ...
    ax.set_xlabel('TRIX rank', fontsize=17)
    ax.set_ylabel('Base rank', fontsize=17)
    ax.tick_params(labelsize=14, width=1.2, length=5)

    # Region labels
    ax.text(2.5, 500, 'Promoted\n(rank ↑)', fontsize=15, color=COLOR_SATISFYING,
            alpha=0.8, fontweight='bold', ha='center', va='center')
    ax.text(400, 2.2, 'Suppressed\n(rank ↓)', fontsize=15, color=COLOR_AFFECTED,
            alpha=0.8, fontweight='bold', ha='center', va='center')
    ...
    legend_elements = [
        plt.Line2D([0], [0], marker='D', color='w', markerfacecolor=COLOR_SATISFYING,
                   markersize=12, label='Relevant doc'),
        plt.Line2D([0], [0], marker='o', color='w', markerfacecolor=COLOR_AFFECTED,
                   markersize=12, label='Excluded doc'),
        plt.Line2D([0], [0], color='gray', linewidth=1.5, linestyle='--', label='No change'),
    ]
    ax.legend(handles=legend_elements, loc='lower left', fontsize=13.5,
