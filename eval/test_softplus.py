...
from model.dsclr_scoring import dsclr_softplus_score, analyze_score_distribution
...
class DSCLRSoftplusEvaluator:
    ...
    def evaluate(
        ...
        logger.info(f"🧠 DSCLR-Softplus 评估 (alpha={alpha}, delta={delta})...")
        ...
        with torch.no_grad():
            ...
            S_final_changed, penalty = dsclr_softplus_score(
                S_base, S_neg, tau, alpha, neg_mask=neg_mask.unsqueeze(1)
            )
    ...
    def grid_search(
        ...
        for alpha in alphas:
            for delta in deltas:
                result = self.evaluate(alpha=alpha, delta=delta, top_k=top_k)
                ...
                if result['map_at_1000'] > best_map:
                    ...
                    best_params = {"alpha": alpha, "delta": delta}
...
if __name__ == "__main__":
    evaluator = DSCLRSoftplusEvaluator(
    ...
    output_dir = "eval/results/core17_softplus"
    os.makedirs(output_dir, exist_ok=True)
    
    alphas = [0.5, 1.0, 1.5, 2.0, 3.0, 5.0]
    deltas = [-0.3, -0.2, -0.15, -0.1, 0.0]
    
    results = evaluator.grid_search(alphas=alphas, deltas=deltas)
