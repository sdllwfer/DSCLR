...
def dsclr_hard_penalty_score(
    ...
    核心思想：当 S_neg > tau 时，使用非常陡峭的惩罚
    
    公式：S_final = S_base - alpha * ReLU(S_neg - tau) - alpha * Softplus(beta * (S_neg - tau)) / beta
    """
    overflow = S_neg - tau.unsqueeze(1)
    
    relu_penalty = torch.relu(overflow)
    soft_penalty = torch.log(1 + torch.exp(beta * overflow)) / beta
    
    penalty = alpha * (relu_penalty + soft_penalty)
...
def dsclr_exponential_penalty_score(
    S_base: torch.Tensor,
    S_neg: torch.Tensor,
    tau: torch.Tensor,
    alpha: float = 1.0,
    ...
    """
    DSCLR-Exponential: 指数惩罚策略
    
    核心思想：当 S_neg > tau 时，使用指数惩罚
    
    公式：S_final = S_base - alpha * (exp(gamma * ReLU(S_neg - tau)) - 1) / gamma
    """
    overflow = torch.relu(S_neg - tau.unsqueeze(1))
    
    penalty = alpha * (torch.exp(gamma * overflow) - 1) / gamma
...
def dsclr_rank_aware_penalty_score(
    S_base: torch.Tensor,
    S_neg: torch.Tensor,
    tau: torch.Tensor,
    alpha: float = 1.0,
    ...
    """
    DSCLR-Rank-Aware: 排名感知惩罚
    
    核心思想：
    - 对于 S_base 高的文档（排名靠前），使用更温和的惩罚
    - 对于 S_base 低的文档（排名靠后），使用更严厉的惩罚
    
    公式：penalty = alpha * (1 + (1 - S_base)^2) * Softplus(S_neg - tau)
    """
    overflow = S_neg - tau.unsqueeze(1)
    
    soft_penalty = torch.log(1 + torch.exp(beta * overflow)) / beta
    
    rank_weight = 1.0 + (1.0 - S_base) ** 2
    
    penalty = alpha * rank_weight * soft_penalty
...
def dsclr_combined_penalty_score(
    S_base: torch.Tensor,
    S_neg: torch.Tensor,
    tau: torch.Tensor,
    alpha: float = 1.0,
    beta: float = 50.0,
    lambda_relu: float = 0.5,
    neg_mask: Optional[torch.Tensor] = None
) -> Tuple[torch.Tensor, torch.Tensor]:
    """
    DSCLR-Combined: 组合惩罚策略
    
    核心思想：结合 ReLU 和 Softplus 的优势
    
    公式：penalty = alpha * (lambda * ReLU(S_neg - tau) + (1-lambda) * Softplus(S_neg - tau))
    """
    overflow = S_neg - tau.unsqueeze(1)
    ...
    penalty = alpha * (lambda_relu * relu_penalty + (1 - lambda_relu) * soft_penalty)
...
class DSCLRAggressiveEvaluator:
    """DSCLR Aggressive 评估器"""
    ...
    def evaluate(
        ...
        with torch.no_grad():
            ...
            if method == "hard":
                S_final_changed, penalty = dsclr_hard_penalty_score(
                    S_base, S_neg, tau, alpha, beta, neg_mask=neg_mask.unsqueeze(1)
                )
            elif method == "exponential":
                S_final_changed, penalty = dsclr_exponential_penalty_score(
                    S_base, S_neg, tau, alpha, gamma, neg_mask=neg_mask.unsqueeze(1)
                )
            elif method == "rank_aware":
                S_final_changed, penalty = dsclr_rank_aware_penalty_score(
                    S_base, S_neg, tau, alpha, beta, neg_mask=neg_mask.unsqueeze(1)
                )
            elif method == "combined":
                S_final_changed, penalty = dsclr_combined_penalty_score(
                    S_base, S_neg, tau, alpha, beta, lambda_relu, neg_mask=neg_mask.unsqueeze(1)
                )
            else:
...
if __name__ == "__main__":
    evaluator = DSCLRAggressiveEvaluator(
    ...
    output_dir = "eval/results/core17_aggressive"
    os.makedirs(output_dir, exist_ok=True)
    
    results = []
    
    print("\n" + "="*60)
    print("测试 Hard Penalty")
    print("="*60)
    for alpha in [1.0, 2.0, 3.0]:
        for delta in [0.0, -0.05, -0.1]:
            for beta in [50, 100]:
                ...
                print(f"  alpha={alpha}, delta={delta}, beta={beta}: MAP={result['map_at_1000']:.4f}, p-MRR={result['p_mrr']:.4f}")
    
    print("\n" + "="*60)
    print("测试 Exponential Penalty")
    print("="*60)
    for alpha in [0.5, 1.0, 2.0]:
        for delta in [0.0, -0.05]:
            for gamma in [3.0, 5.0, 10.0]:
                result = evaluator.evaluate(
                    method="exponential",
                    alpha=alpha,
                    delta=delta,
                ...
                results.append(result)
                print(f"  alpha={alpha}, delta={delta}, gamma={gamma}: MAP={result['map_at_1000']:.4f}, p-MRR={result['p_mrr']:.4f}")
    
    print("\n" + "="*60)
    print("测试 Rank-Aware Penalty")
    print("="*60)
    ...
    print("\n" + "="*60)
    print("测试 Combined Penalty")
    print("="*60)
    for alpha in [1.0, 2.0]:
        for lambda_relu in [0.3, 0.5, 0.7, 0.9]:
            for delta in [0.0, -0.05]:
                result = evaluator.evaluate(
                    method="combined",
                    alpha=alpha,
                ...
                results.append(result)
                print(f"  alpha={alpha}, lambda={lambda_relu}, delta={delta}: MAP={result['map_at_1000']:.4f}, p-MRR={result['p_mrr']:.4f}")
