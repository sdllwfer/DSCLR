#!/usr/bin/env python
"""测试 DSCLR-Softplus with different beta"""
...
def dsclr_softplus_v2_score(
    ...
    """
    DSCLR-Softplus V2: 更陡峭的 Softplus
    
    公式：S_final = S_base - alpha * Softplus(beta * (S_neg - tau)) / beta
    """
...
def dsclr_adaptive_penalty_score(
    S_base: torch.Tensor,
    S_neg: torch.Tensor,
    tau: torch.Tensor,
    alpha: float = 1.0,
    ...
    """
    DSCLR-Adaptive: 自适应惩罚
    
    核心思想：
    - 对于 S_base 高的文档（好文档），使用更温和的惩罚
    - 对于 S_base 低的文档（差文档），使用更严厉的惩罚
    
    公式：penalty = alpha * (1 - S_base) * Softplus(S_neg - tau)
    """
    overflow = S_neg - tau.unsqueeze(1)
    
    base_penalty = torch.log(1 + torch.exp(beta * overflow)) / beta
    
    adaptive_weight = 1.0 + (1.0 - S_base)
    
    penalty = alpha * adaptive_weight * base_penalty
...
def dsclr_double_threshold_score(
    ...
    """
    DSCLR-Double-Threshold: 双阈值惩罚
    
    核心思想：
    - 当 S_neg < tau 时：不惩罚
    - 当 tau <= S_neg < tau + margin 时：温和惩罚
    - 当 S_neg >= tau + margin 时：严厉惩罚
    
    公式：
    - 如果 S_neg < tau: penalty = 0
    - 如果 tau <= S_neg < tau + margin: penalty = alpha * (S_neg - tau)
    - 如果 S_neg >= tau + margin: penalty = alpha * margin + alpha * 2 * (S_neg - tau - margin)
    """
    tau_expanded = tau.unsqueeze(1)
    upper_tau = tau_expanded + margin
    
    overflow = S_neg - tau_expanded
    
    soft_penalty = torch.log(1 + torch.exp(beta * overflow)) / beta
    
    hard_overflow = S_neg - upper_tau
    hard_overflow = torch.relu(hard_overflow)
    
    penalty = alpha * soft_penalty + alpha * hard_overflow
...
class DSCLREnhancedEvaluator:
    ...
    def evaluate(
        ...
        logger.info(f"🧠 DSCLR-{method} 评估 (alpha={alpha}, delta={delta}, beta={beta})...")
        ...
        with torch.no_grad():
            ...
            if method == "softplus_v2":
                S_final_changed, penalty = dsclr_softplus_v2_score(
                    S_base, S_neg, tau, alpha, beta, neg_mask=neg_mask.unsqueeze(1)
                )
            elif method == "adaptive":
                S_final_changed, penalty = dsclr_adaptive_penalty_score(
                    S_base, S_neg, tau, alpha, beta, neg_mask=neg_mask.unsqueeze(1)
                )
            ...
            protected_count = (S_neg <= tau.unsqueeze(1)).sum().item()
            total_count = S_neg.numel()
            protected_ratio = protected_count / total_count
            
            logger.info(f"   得分分布: protected={protected_ratio:.2%}, penalized={1-protected_ratio:.2%}")
...
if __name__ == "__main__":
    evaluator = DSCLREnhancedEvaluator(
    ...
    output_dir = "eval/results/core17_enhanced"
    os.makedirs(output_dir, exist_ok=True)
    
    results = []
    
    print("\n" + "="*60)
    print("测试 Softplus V2 (更陡峭的 beta)")
    print("="*60)
    for beta in [20, 50, 100, 200]:
        for alpha in [1.0, 2.0]:
            for delta in [0.0, -0.05, -0.1]:
                ...
                print(f"  beta={beta}, alpha={alpha}, delta={delta}: MAP={result['map_at_1000']:.4f}, p-MRR={result['p_mrr']:.4f}")
    
    print("\n" + "="*60)
    print("测试 Adaptive Penalty")
    print("="*60)
    ...
    for alpha in [1.0, 2.0]:
        for margin in [0.02, 0.05, 0.1]:
            for delta in [0.0, -0.05]:
                ...
                results.append(result)
                print(f"  alpha={alpha}, margin={margin}, delta={delta}: MAP={result['map_at_1000']:.4f}, p-MRR={result['p_mrr']:.4f}")
    ...
    for r in sorted_by_map[:10]:
        print(f"  {r['method']}: alpha={r['alpha']}, delta={r['delta']}, beta={r.get('beta', 'N/A')}, margin={r.get('margin', 'N/A')}: MAP={r['map_at_1000']:.4f}, p-MRR={r['p_mrr']:.4f}")
